...
let mut files = match files_mutex.lock() {
    ...
    pub(crate) fn add_or_update_entry(&mut self, file_name: String, hash: Option<String>, short_url: Option<String>, post_id: Option<i64>) {
        ...
        // Use cached database if available, otherwise load from disk
        let mut hash_db = if let Some(ref cached_db) = self.cached_database {
            cached_db.clone()
        } else {
        ...
        // No need to refresh post_id_cache since we've already updated it above
    ...
    pub(crate) fn batch_has_post_ids(&self, post_ids: &[i64]) -> Vec<(i64, bool)> {
        ...
        if let Some(ref blacklist) = self.cached_blacklist {
            ...
            for (post_id, is_duplicate) in &mut results {
                ...
                if !*is_duplicate {
                    ...
                    for filename in &potential_filenames {
                        if blacklist.is_blacklisted(filename) {
                            trace!("Post ID {} has potential filename {} which is blacklisted", post_id, filename);
                            *is_duplicate = true;
                            break;
                        }
                    }
