...
use crate::e621::io::{emergency_exit, Config};
...
pub(crate) struct Tag {
    ...
    tag_type: TagType,
    /// Directory associated with this tag
...
impl Tag {
    ...
    pub(crate) fn initialize_directory(&mut self) -> Result<(), Error> {
        ...
        self.directory = Some(match self.tag_type {
            ...
            TagType::Pool => dir_manager.get_pool_directory(&self.name)?,
            _ => dir_manager.get_tag_directory(&self.name)?,
        });

        Ok(())
    }

    /// Get the directory associated with this tag
...
impl TagIdentifier {
    ...
    fn exit_tag_failure(&self, tag: &str) {
        ...
        emergency_exit(&format!("The server API call was unable to find tag: {}!", tag));
    }
