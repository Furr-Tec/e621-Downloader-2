...
pub async fn orchestration_example() -> Result<()> {
    ...
    let _query1_id = orchestrator.add_query(vec!["wolf".to_string(), "solo".to_string()], 1).await?;
    let _query2_id = orchestrator.add_query(vec!["dragon".to_string(), "male".to_string()], 2).await?;
