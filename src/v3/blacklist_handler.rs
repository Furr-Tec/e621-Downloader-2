...
use crate::v3::{
    AppConfig, ConfigManager, ConfigResult, RulesConfig,
...
impl BlacklistHandler {
    ...
    pub fn new(config_manager: Arc<ConfigManager>, db_path: &Path) -> BlacklistHandlerResult<Self> {
    ...
    pub fn from_config(config_manager: Arc<ConfigManager>) -> BlacklistHandlerResult<Self> {
        // Get the app config
        let app_config = config_manager.get_app_config()
            .map_err(|e| BlacklistHandlerError::Config(e.to_string()))?;
    ...
    pub fn add_blacklisted_reject(&self, post_id: u32) -> BlacklistHandlerResult<()> {
    ...
    pub fn mark_as_deleted(&self, post_id: u32) -> BlacklistHandlerResult<()> {
    ...
    pub fn is_deleted_blacklisted_reject(&self, post_id: u32) -> BlacklistHandlerResult<bool> {
        // Query the database
    ...
    pub fn get_all_blacklisted_rejects(&self) -> BlacklistHandlerResult<Vec<BlacklistedReject>> {
        // Query the database
        let mut stmt = self.db_connection.prepare(
...
pub async fn init_blacklist_handler(
    ...
    let handler = BlacklistHandler::from_config(config_manager)?;
    Ok(Arc::new(handler))
}